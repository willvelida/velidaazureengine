terraform {
    backend "azure" {
      ...
      container_name = "myhealthfitbitactivitytfstate"
...
module "resource_group" {
    ...
    resource_group_tags = {
        ...
        "ServiceName" = "MyHealth.Fitbit.Activity"
    }  
}
...
resource "azurerm_function_app" "fitbitactivity" {
  name = var.myhealth_fitbit_activity_function_name
...
resource "azurerm_key_vault_access_policy" "velidakeyvault_policy" {
  ...
  object_id = azurerm_function_app.fitbitactivity.identity[0].principal_id
