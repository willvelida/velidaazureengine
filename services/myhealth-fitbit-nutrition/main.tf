terraform {
    ...
    backend "azure" {
      ...
      container_name = "myhealthfitbitnutritiontfstate"
...
module "resource_group" {
    ...
    resource_group_tags = {
        ...
        "ServiceName" = "MyHealth.Fitbit.Nutrition"
    }  
}
...
# Create storage account for MyHealth.Fitbit.Sleep
...
# Create Function App for MyHealth.Fitbit.Sleep
resource "azurerm_function_app" "fitbitnutrition" {
  name = var.myhealth_fitbit_nutrition_function_name
...
resource "azurerm_key_vault_access_policy" "velidakeyvault_policy" {
  ...
  object_id = azurerm_function_app.fitbitnutrition.identity[0].principal_id
