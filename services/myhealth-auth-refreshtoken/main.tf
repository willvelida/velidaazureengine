terraform {
    ...
    backend "azure" {
      ...
      container_name = "myhealthauthrefreshtfstate"
...
module "resource_group" {
    ...
    resource_group_tags = {
        ...
        "ServiceName" = "MyHealth.Auth.RefreshToken"
    }  
}
...
resource "azurerm_function_app" "authrefresh" {
  name = var.myhealth_auth_refreshtoken_function_name
...
resource "azurerm_key_vault_access_policy" "velidakeyvault_policy" {
  ...
  object_id = azurerm_function_app.authrefresh.identity[0].principal_id
