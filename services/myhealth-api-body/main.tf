terraform {
    ...
    backend "azure" {
      ...
      container_name = "myhealthapibodytfstate"
...
# Create storage account for MyHealth.API.Body
...
resource "azurerm_function_app" "bodyapi" {
  name = var.myhealth_api_body_function_name
...
resource "azurerm_key_vault_access_policy" "velidakeyvault_policy" {
  ...
  object_id = azurerm_function_app.bodyapi.identity[0].principal_id
