terraform {
    backend "azure" {
      ...
      container_name = "myhealthfilevalidatorsleeptfstate"
...
module "resource_group" {
    ...
    resource_group_tags = {
        ...
        "ServiceName" = "MyHealth.FileValidator.Sleep"
    }  
}
...
# Create storage account for MyHealth.FileValidator.Sleep
...
# Create Function App for MyHealth.FileValidator.Sleep
resource "azurerm_function_app" "myhealthfvsleep" {
  name = var.myhealth_filevalidator_sleep_function_name
...
resource "azurerm_key_vault_access_policy" "velidakeyvault_policy" {
  ...
  object_id = azurerm_function_app.myhealthfvsleep.identity[0].principal_id
  secret_permissions = [ "get","list" ]
}

# Create Activity Topic in Service Bus Namespace
module "sleep_sb_topic" {
