terraform {
    ...
    backend "azure" {
      ...
      container_name = "myhealthapiexercisetfstate"
...
module "resource_group" {
    ...
    resource_group_tags = {
        ...
        "ServiceName" = "MyHealth.API.Exercise"
    }  
}
...
# Create storage account for MyHealth.API.Exercise
...
# Create Function App for MyHealth.API.Exercise
resource "azurerm_function_app" "exerciseapi" {
  name = var.myhealth_api_exercise_function_name
...
resource "azurerm_key_vault_access_policy" "velidakeyvault_policy" {
  ...
  object_id = azurerm_function_app.exerciseapi.identity[0].principal_id
