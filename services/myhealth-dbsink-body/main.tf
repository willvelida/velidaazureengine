terraform {
    backend "azure" {
      ...
      container_name = "myhealthdbsinkbodytfstate"
...
module "resource_group" {
    ...
    resource_group_tags = {
        ...
        "ServiceName" = "MyHealth.DBSink.Body"
    }  
}
...
# Create storage account for MyHealth.DBSink.Body
...
# Create Function App for MyHealth.DBSink.Body
resource "azurerm_function_app" "dbsinkbody" {
  name = var.myhealth_dbsink_body_function_name
...
resource "azurerm_key_vault_access_policy" "velidakeyvault_policy" {
  ...
  object_id = azurerm_function_app.dbsinkbody.identity[0].principal_id
