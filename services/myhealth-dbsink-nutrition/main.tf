terraform {
    backend "azure" {
      ...
      container_name = "myhealthdbsinknutritiontfstate"
...
module "resource_group" {
    ...
    resource_group_tags = {
        ...
        "ServiceName" = "MyHealth.DBSink.Nutrition"
    }  
}
...
# Create storage account for MyHealth.DBSink.Nutrition
...
# Create Function App for MyHealth.DBSink.Nutrition
resource "azurerm_function_app" "dbsinknutrition" {
  name = var.myhealth_dbsink_nutrition_function_name
...
resource "azurerm_key_vault_access_policy" "velidakeyvault_policy" {
  ...
  object_id = azurerm_function_app.dbsinknutrition.identity[0].principal_id
