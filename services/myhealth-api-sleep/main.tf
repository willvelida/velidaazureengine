terraform {
    ...
    backend "azure" {
      ...
      container_name = "myhealthapisleeptfstate"
...
module "resource_group" {
    ...
    resource_group_tags = {
        ...
        "ServiceName" = "MyHealth.API.Sleep"
    }  
}
...
# Create storage account for MyHealth.API.Sleep
...
# Create Function App for MyHealth.API.Sleep
resource "azurerm_function_app" "sleepapi" {
  name = var.myhealth_api_sleep_function_name
...
resource "azurerm_key_vault_access_policy" "velidakeyvault_policy" {
  ...
  object_id = azurerm_function_app.sleepapi.identity[0].principal_id
