terraform {
    backend "azure" {
      ...
      container_name = "myhealthdbsinksleeptfstate"
...
module "resource_group" {
    ...
    resource_group_tags = {
        ...
        "ServiceName" = "MyHealth.DBSink.Sleep"
    }  
}
...
# Create storage account for MyHealth.DBSink.Sleep
...
# Create Function App for MyHealth.DBSink.Sleep
resource "azurerm_function_app" "myhealthdbsleep" {
  name = var.myhealth_dbsink_sleep_function_name
...
resource "azurerm_key_vault_access_policy" "velidakeyvault_policy" {
  ...
  object_id = azurerm_function_app.myhealthdbsleep.identity[0].principal_id
